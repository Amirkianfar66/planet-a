...
// Optional: export a door component for GameCanvas if you want
...
function computeAABBs(arr) { return (arr || []).map(w => ({ minX: w.x - w.w / 2, maxX: w.x + w.w / 2, minZ: w.z - w.d / 2, maxZ: w.z + w.d / 2 })); }
...
function normalizeMap(m) {
    ...
    map.doors = Array.isArray(map.doors) ? map.doors : [];
    return map;
}
...
function getAuthoredMap() {
    try {
        if (typeof window !== "undefined") {
            ...
            if (raw3) { const p3 = JSON.parse(raw3); if (p3 && Array.isArray(p3.rooms)) return normalizeMap(p3); }
            const raw2 = localStorage.getItem("mapEditorDraft_v2");
            if (raw2) { const p2 = JSON.parse(raw2); if (p2 && Array.isArray(p2.rooms)) return normalizeMap(p2); }
        }
    } catch { }
...
// Normalize doors (defaults + side→yaw)
...
function normalizeDoors(rawDoors, rooms) {
    ...
    for (const d of (rawDoors || [])) {
        ...
        const rotY = Number.isFinite(d.rotY) ? (Math.abs(d.rotY) > 1e-6 ? d.rotY : sideYaw + roomRot) : (sideYaw + roomRot);
...
// Base walls (baked or synthesized)
...
if (!packedWalls.walls.length && typeof RT.packMap === "function") {
    ...
    } catch (e) { console.warn("packMap failed; using empty walls", e); }
}

// --- Split walls around doors (1 door max per edge) ---
function splitWallsByDoors(wallsIn, doors, rooms) {
    ...
    for (const w of (wallsIn || [])) {
        ...
        // Wall base dims/orientation:
        const isH = (w.side === "N" || w.side === "S"); // along X
        ...
        // door slot
        const slotW = (Number(d.width) && d.width > 0) ? Number(d.width) : DEFAULT_DOOR_WIDTH;
        const offs = Number.isFinite(d.offset) ? Number(d.offset)
            : (isH ? (d.x - w.x) : (d.z - w.z)); // project world center to local axis
...
// --- Floors/Roofs synthesis (fallback when JSON not provided) ---
...
export function pointInRoom(room, x, z) {
    ...
    const dx = x - room.x, dz = z - room.z, lx = dx * cos + dz * sin, lz = -dx * sin + dz * cos;
...
function findMeetingRoom(rooms) {
    ...
    return rooms.find(r => lower(r.key) === "meeting_room" || lower(r.type) === "meeting_room" || lower(r.name).includes("meeting")) || null;
}
