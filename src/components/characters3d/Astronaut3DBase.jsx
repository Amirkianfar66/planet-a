...
export default function Astronaut3DBase({
  suit, prop, name, role, showName, bob, speed, airborne,
    ...
    useFrame((state) => {
        ...
        // idle bob (disabled while walking or in air)
        if (!moving && !airborne && bob && root.current) {
        ...
        // walking cycle
        if (moving && !airborne) {
            // swing frequency scales with speed; clamp to reasonable range
            const w = THREE.MathUtils.clamp(3.5 + speed * 0.8, 3.5, 9.5);
            const phase = t * w;
            const legAmp = 0.6;     // ~34°
            const armAmp = 0.35;    // ~20°
            ...
            // reset swings when not moving on ground
            if (!airborne) {
        ...
        if (airborne) {
            ...
            if (root.current) root.current.rotation.x = 0.05; // slight forward lean
        } else {
            ...
            case "gun":
                return (
                    <group position={[0.6, 0.95, 0.18]} rotation={[0, -Math.PI / 10, 0]}>
                        <mesh><boxGeometry args={[0.34, 0.16, 0.12]} /><meshStandardMaterial color={secondary} /></mesh>
                        <mesh position={[-0.08, -0.18, 0]}><boxGeometry args={[0.12, 0.24, 0.1]} /><meshStandardMaterial color={secondary} /></mesh>
                        <mesh position={[0.12, 0, 0]}><boxGeometry args={[0.16, 0.06, 0.1]} /><meshStandardMaterial color={shadeHex(secondary, -30)} /></mesh>
                    </group>
                );
            case "backpack":
                return (
