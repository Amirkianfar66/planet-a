...
export default function SimplePickupDemo() {
  ...
  useEffect(() => {
    const t = setInterval(() => {
      try {
        ...
        const anyPicked = arr.some(p => String(p?.getState("demoPicked") || "") === ITEM_ID);
        // only set state when it changes to avoid re-render spam
  ...
  // Host: process pickup requests at a gentle cadence (no queueMicrotask)
  useEffect(() => {
    if (!host) return;
